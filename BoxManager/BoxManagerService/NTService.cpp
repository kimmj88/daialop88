...
// last revised: $Date: 08-12-22 9:30a $, $Revision: 1 $
...
BOOL CNTService :: RegisterService( int argc, char ** argv ) {
    ...
    while( ++Argv, --Argc ) {
		if( Argv[0][0] == TEXT('-') ) {
			switch( Argv[0][1] ) {
				case TEXT('t'):	// service tag for multiple installation
					m_pszServiceTag = NEXT_ARG;
					sprintf(m_szServiceName, "%s%s", m_lpServiceName, m_pszServiceTag);
					m_lpServiceName = m_szServiceName;
					sprintf(m_szDisplayName, "%s (%s)", m_lpDisplayName, m_pszServiceTag);
					m_lpDisplayName = m_szDisplayName;
					break;
				...
				case TEXT('c'):	// configure the service
					fnc = &CNTService::ConfigureService;
...
BOOL CNTService :: StartupService() {
	...
	if( schSCManager ) 
	{
		SC_HANDLE schService =	::OpenService(schSCManager, m_lpServiceName, SC_MANAGER_ALL_ACCESS);
...
BOOL CNTService :: ConfigureService() {
	return FALSE;
}
...
void WINAPI CNTService :: ServiceMain(DWORD dwArgc, LPTSTR *lpszArgv) {
	...
	gpTheService->m_pszServiceTag = lpszArgv[0] + strlen(gpTheService->m_lpServiceName);
	gpTheService->m_lpServiceName = lpszArgv[0];
	if (gpTheService->m_pszServiceTag && *gpTheService->m_pszServiceTag)
	{
		sprintf(gpTheService->m_szDisplayName, "%s (%s)", gpTheService->m_lpDisplayName, gpTheService->m_pszServiceTag);
		gpTheService->m_lpDisplayName = gpTheService->m_szDisplayName;
	}
